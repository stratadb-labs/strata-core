...
fn test_search_completes_within_reasonable_time() {
    ...
    let _ = vector.search(test_db.branch_id, "embeddings", &query, 100, None).unwrap();
...
fn test_search_with_various_k_values() {
    ...
    for k in [1, 10, 50, 100] {
        let results = vector.search(test_db.branch_id, "embeddings", &query, k, None).unwrap();
...
fn test_k_larger_than_collection() {
    ...
    let query = seeded_random_vector(384, 999);
    let results = vector.search(test_db.branch_id, "embeddings", &query, 100, None).unwrap();

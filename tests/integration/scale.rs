...
mod event_scale {
    ...
    fn run_event_scale_test(count: usize) {
        ...
        measure(&format!("Count {} events", count), || {
            let len = event.len_by_type(&run_id, "scale_test").unwrap();
...
mod vector_scale {
    ...
    fn run_vector_scale_test(count: usize) {
        ...
        // Count
        let actual_count = vector.count(run_id, "scale_test").unwrap();
...
fn cross_primitive_scale_1k() {
    ...
    assert_eq!(p.event.len_by_type(&run_id, "items").unwrap(), count as u64);
    assert_eq!(p.vector.count(run_id, "embeddings").unwrap(), count);
}
...
fn large_kv_values() {
    ...
    if let Value::Bytes(b) = val.value {

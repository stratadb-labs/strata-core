...
/// Snapshot entry for Trace primitive
///
/// Format: trace_id_len(4) + trace_id + parent_len(4) + parent + spans_count(4) + [spans...]
/// Span format: span_id_len(4) + span_id + name_len(4) + name + start(8) + end(8) + attrs_len(4) + attrs
#[derive(Debug, Clone, PartialEq)]
pub struct TraceSnapshotEntry {
    /// Trace identifier
    pub trace_id: String,
    /// Parent trace ID (empty if root)
    pub parent: String,
    /// Spans in this trace
    pub spans: Vec<SpanSnapshotEntry>,
}

/// Span within a trace
#[derive(Debug, Clone, PartialEq)]
pub struct SpanSnapshotEntry {
    /// Span identifier
    pub span_id: String,
    /// Span name
    pub name: String,
    ...
    pub start_time: u64,
    /// End timestamp (microseconds, 0 if not ended)
    pub end_time: u64,
    /// Attributes as serialized bytes
...
impl SnapshotSerializer {
    ...
    /// Serialize Trace entries to bytes
    pub fn serialize_traces(&self, entries: &[TraceSnapshotEntry]) -> Vec<u8> {
        let mut data = Vec::new();
        ...
        for entry in entries {
            // Trace ID
            let trace_id_bytes = entry.trace_id.as_bytes();
            data.extend_from_slice(&(trace_id_bytes.len() as u32).to_le_bytes());
            data.extend_from_slice(trace_id_bytes);

            // Parent
            let parent_bytes = entry.parent.as_bytes();
            data.extend_from_slice(&(parent_bytes.len() as u32).to_le_bytes());
            data.extend_from_slice(parent_bytes);

            // Spans
            data.extend_from_slice(&(entry.spans.len() as u32).to_le_bytes());
            for span in &entry.spans {
                let span_id_bytes = span.span_id.as_bytes();
                data.extend_from_slice(&(span_id_bytes.len() as u32).to_le_bytes());
                data.extend_from_slice(span_id_bytes);

                let name_bytes = span.name.as_bytes();
                data.extend_from_slice(&(name_bytes.len() as u32).to_le_bytes());
                data.extend_from_slice(name_bytes);

                data.extend_from_slice(&span.start_time.to_le_bytes());
                data.extend_from_slice(&span.end_time.to_le_bytes());

                let attrs_bytes = self.codec.encode(&span.attributes);
                data.extend_from_slice(&(attrs_bytes.len() as u32).to_le_bytes());
                data.extend_from_slice(&attrs_bytes);
            }
        }
    ...
    /// Deserialize Trace entries from bytes
    pub fn deserialize_traces(&self, data: &[u8]) -> Result<Vec<TraceSnapshotEntry>, PrimitiveSerializeError> {
        let mut cursor = 0;
        ...
        for _ in 0..count {
            ...
            let trace_id_len = u32::from_le_bytes(data[cursor..cursor + 4].try_into().unwrap()) as usize;
            cursor += 4;

            if cursor + trace_id_len > data.len() {
                return Err(PrimitiveSerializeError::UnexpectedEof);
            }
            let trace_id = String::from_utf8(data[cursor..cursor + trace_id_len].to_vec())
                .map_err(|_| PrimitiveSerializeError::InvalidUtf8)?;
            cursor += trace_id_len;
            ...
            let parent_len = u32::from_le_bytes(data[cursor..cursor + 4].try_into().unwrap()) as usize;
            cursor += 4;

            if cursor + parent_len > data.len() {
                return Err(PrimitiveSerializeError::UnexpectedEof);
            }
            let parent = String::from_utf8(data[cursor..cursor + parent_len].to_vec())
                .map_err(|_| PrimitiveSerializeError::InvalidUtf8)?;
            cursor += parent_len;
            ...
            let spans_count = u32::from_le_bytes(data[cursor..cursor + 4].try_into().unwrap()) as usize;
            cursor += 4;
            ...
            for _ in 0..spans_count {
                ...
                let span_id_len = u32::from_le_bytes(data[cursor..cursor + 4].try_into().unwrap()) as usize;
                cursor += 4;

                if cursor + span_id_len > data.len() {
                    return Err(PrimitiveSerializeError::UnexpectedEof);
                }
                let span_id = String::from_utf8(data[cursor..cursor + span_id_len].to_vec())
                    .map_err(|_| PrimitiveSerializeError::InvalidUtf8)?;
                cursor += span_id_len;
                ...
                let start_time = u64::from_le_bytes(data[cursor..cursor + 8].try_into().unwrap());
                cursor += 8;
                let end_time = u64::from_le_bytes(data[cursor..cursor + 8].try_into().unwrap());
                cursor += 8;
                ...
                let attrs_len = u32::from_le_bytes(data[cursor..cursor + 4].try_into().unwrap()) as usize;
                cursor += 4;

                if cursor + attrs_len > data.len() {
                    return Err(PrimitiveSerializeError::UnexpectedEof);
                }
                let attributes = self.codec.decode(&data[cursor..cursor + attrs_len])?;
                cursor += attrs_len;

                spans.push(SpanSnapshotEntry {
                    span_id,
                    name,
                    start_time,
                    end_time,
            ...
            entries.push(TraceSnapshotEntry {
                trace_id,
                parent,
                spans,
            });
...
mod tests {
    ...
    fn test_traces_roundtrip() {
        ...
        let entries = vec![TraceSnapshotEntry {
            ...
            spans: vec![
                ...
                SpanSnapshotEntry {
                    ...
                    start_time: 1500,
                    end_time: 1800,
                    attributes: b"{\"key\":\"value\"}".to_vec(),
                },
            ],
        }];

        let data = serializer.serialize_traces(&entries);
        let parsed = serializer.deserialize_traces(&data).unwrap();

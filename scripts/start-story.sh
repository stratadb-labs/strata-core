...
# M7 Epics
EPIC_NAMES[40]="snapshot-foundation"
EPIC_NAMES[41]="snapshot-writer"
EPIC_NAMES[42]="snapshot-reader"
EPIC_NAMES[43]="wal-truncation"
EPIC_NAMES[44]="recovery-integration"
EPIC_NAMES[45]="validation"
# M8 Epics
EPIC_NAMES[46]="hnsw-core"
EPIC_NAMES[47]="hnsw-persistence"
EPIC_NAMES[48]="hnsw-search"
EPIC_NAMES[49]="vector-transactions"
EPIC_NAMES[50]="vector-wal"
EPIC_NAMES[51]="vector-recovery"
EPIC_NAMES[52]="vector-snapshot"
EPIC_NAMES[53]="filtering"
EPIC_NAMES[54]="batch-operations"
EPIC_NAMES[55]="m8-benchmarks"
EPIC_NAMES[56]="graph-persistence"
EPIC_NAMES[57]="incremental-persistence"
EPIC_NAMES[58]="graph-recovery"
EPIC_NAMES[59]="m8-validation"
# M9 Epics
EPIC_NAMES[60]="core-types"
EPIC_NAMES[61]="versioned-returns"
EPIC_NAMES[62]="transaction-unification"
EPIC_NAMES[63]="error-standardization"
EPIC_NAMES[64]="conformance-testing"
...
elif [ "$EPIC_NUM" -ge 60 ] && [ "$EPIC_NUM" -le 64 ]; then
    echo ""
    echo "🔴 M9 REMINDER: Read docs/architecture/M9_ARCHITECTURE.md"
    echo "   THIS IS THE AUTHORITATIVE SPEC. No deviations allowed."
    echo ""
    echo "   Story specs: docs/milestones/M9/EPIC_${EPIC_NUM}_*.md"
    echo "   Implementation prompts: docs/prompts/M9/epic-${EPIC_NUM}-claude-prompts.md"
    echo ""
    echo "   ⚠️  NAMING CONVENTION - CRITICAL:"
    echo "   NEVER use 'M9' or 'Strata' in codebase or comments!"
    echo "   Use 'in-mem' as the database name consistently."
    echo ""
    echo "   The FOUR ARCHITECTURAL RULES are NON-NEGOTIABLE:"
    echo "   1. Every Read Returns Versioned<T> - no raw values without version info"
    echo "   2. Every Write Returns Version - every mutation returns version created"
    echo "   3. Transaction Trait Covers All Primitives - TransactionOps for all"
    echo "   4. Run Scope Is Always Explicit - no ambient run context"
    echo ""
    echo "   The SEVEN INVARIANTS (all primitives must conform):"
    echo "   1. Addressable - EntityRef for stable identity"
    echo "   2. Versioned - Versioned<T> reads, Version writes"
    echo "   3. Transactional - participates in transactions"
    echo "   4. Lifecycle - create/exist/evolve/destroy"
    echo "   5. Run-scoped - belongs to exactly one run"
    echo "   6. Introspectable - has exists() or equivalent"
    echo "   7. Read/Write - reads don't modify, writes produce versions"
    echo ""
    echo "   See also: docs/prompts/M9/M9_PROMPT_HEADER.md"
fi
